...
const LoginForm = () => {
  ...
  const handleQuickLogin = async (email: string, password: string) => {
    ...
    await form.handleSubmit(onSubmit)(); // trigger submit
  };

  return (
    <div className="flex min-h-[calc(100vh-80px)] items-center justify-center flex-col space-y-4">
      {/* Quick login buttons */}
      <div className="flex space-x-4 mb-2">
        <Button
          variant="outline"
          onClick={() => handleQuickLogin("jpchoyonkhan65@gmail.com", "123456")}
        >
          Login as User
        </Button>
        <Button
          ...
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-7">
            <CardContent className="space-y-6">
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    <FormLabel className="text-black/85">
                      Email or Phone
                    </FormLabel>
